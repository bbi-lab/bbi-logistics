...
def main():
	...
	doc.update('A2', datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d %H:%M'))
...
def getZipcodes(needZip, project):
	...
	data = {
		...
		'fields':",".join(map(str, zipFields)),
		'rawOrLabel': 'label',
		'returnFormat': 'json',
		'records':",".join(map(str, needZip))
	}
...
def getRecords(project, date, zipcode_county_map):
	...
	data = {
		...
		'format': 'json',
		'type': 'flat',
		'events':",".join(map(str, projectEvents)),
		'fields':",".join(map(str, formattedFields)),
		'rawOrLabel': 'label',
		'returnFormat': 'json',
		'filterLogic': '[event-name]['+str(projectDict[project]['BEMS'])+'] >= "'+str(date)+'"'
	}
	r = requests.post(url.geturl(),data=data)
	results = r.json()
	print('{: <30}{: <30}'.format(str(project)+' shipped:',str(len(results))))
	if len(results) == 0:
		return(results)
	...
	records[zipcode] = records[zipcode].apply(lambda x: re.findall('[0-9][0-9][0-9][0-9][0-9]', x)[0] if re.search('^<', x) else x)

	#assign project name to records
	if project in ('HCT', 'Childcare', 'SSD'):
		records['project'] = project
	elif re.search('SCAN+', project):
		records['project'] = records[zipcode].apply(lambda x: getScanProject(x, zipcode_county_map))
