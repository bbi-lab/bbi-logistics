...
def get_pc_redcap_data():
    export_feilds = [
        'call_date', 'study', 'highlevel_sub', 'enrollment_sub', 'redcap_sub',
        'shipping_sub', 'testing_sub', 'results_sub', 'feedback_sub',
        'time_fu', 'cascadia_ptid'
    ]
...
def import_pc(data, sheet):
    ...
    data = data.apply(cascadia_regions, axis=1)
    null_issues = data.loc[(data['highlevel_sub'].isnull()) &
                           ...
                           (data['results_sub'].isnull()),
                           ['call_date', 'time_fu', 'study']]
    null_issues[['category', 'issue']] = [pd.NA, pd.NA]

    data = pd.melt(data,
                   id_vars=['call_date', 'time_fu', 'study'],
                   value_vars=[
                       ...
                       'variable': 'category',
                       'value': 'issue'
                   }).dropna(subset=['issue']).append(null_issues)

    follow_up_data = data.dropna(subset=['time_fu']).drop(
        'call_date', axis=1).rename(columns={'time_fu': 'call_date'})
    follow_up_data['call_date'] = follow_up_data.apply(
        lambda x: pd.to_datetime(x['call_date']).strftime('%Y-%m-%d'), axis=1)

    data = data.drop('time_fu', axis=1).append(follow_up_data).fillna('')
...
def cascadia_regions(row):
    if pd.isna(row['study']) or row['study'] != 'CASCADIA':
        return row
    elif pd.isna(row['cascadia_ptid']):
        row['study'] = 'Cascadia'
    elif row['cascadia_ptid'][0] == '2':
        row['study'] = 'Cascadia_SEA'
    elif row['cascadia_ptid'][0] == '1':
        row['study'] = 'Cascadia_PDX'
...
def get_ge_redcap_data():
    export_feilds = [
        'consent_date', 'attempt_1', 'attempt_2', 'attempt_3', 'referral_date'
    ]
    url = urlparse(os.environ.get("REDCAP_API_URL"))
...
def import_ge(data, sheet):
    attempts = pd.concat(
        [data['attempt_1'], data['attempt_2'], data['attempt_3']],
        ignore_index=True).value_counts(dropna=True)
    attmepts = pd.DataFrame(attempts.reset_index()).rename(columns={
        'index': 'date',
        0: 'attempts'
    })
    enrollments = pd.DataFrame(data['consent_date'].value_counts(
        dropna=True).reset_index()).rename(columns={
            'index': 'date',
            'consent_date': 'enrollments'
        })
    referrals = pd.DataFrame(data['referral_date'].value_counts(
        dropna=True).reset_index()).rename(columns={
            'index': 'date',
            'referral_date': 'referrals'
        })
    ...
    try:
        sheet.append_rows(df_final.fillna(0).values.tolist(),
                          value_input_option='USER_ENTERED')
    except Exception as e:
        print(f'Error inserting data {e}')
        sheet.append_rows([['']])
